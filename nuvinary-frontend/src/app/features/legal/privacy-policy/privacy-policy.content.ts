...
<p>
  ...
  die &uuml;bermittelten Daten auch f&uuml;r Vertragsangebote, Bestellungen oder sonstige
...
<hr />
<h3>Ergänzende AWS-Dienste (CloudFront, S3, Bedrock)</h3>
<p>Wir nutzen im Rahmen des Hostings weitere spezifische Dienste von Amazon Web Services (AWS):</p>
<p>
  <strong>Amazon CloudFront (CDN):</strong> Wir nutzen das Content Delivery Network (CDN) Amazon
  CloudFront. Dies optimiert die Ladegeschwindigkeit unserer Website, indem Inhalte &uuml;ber
  weltweite Server verteilt werden. Die Nutzung erfolgt auf Grundlage unseres berechtigten
  Interesses an einer schnellen und sicheren Bereitstellung unseres Angebots (Art. 6 Abs. 1 lit. f
  DSGVO).
</p>
<p>
  <strong>Amazon S3:</strong> Zur Speicherung von Nutzerdaten (z. B. Profilbilder) und den
  generierten Inhalten nutzen wir den Cloud-Speicherdienst Amazon S3. Die Daten werden
  verschl&uuml;sselt auf den Servern von AWS gespeichert. Dies dient der Vertragserf&uuml;llung
  gegenüber unseren Nutzern (Art. 6 Abs. 1 lit. b DSGVO).
</p>
<p>
  <strong>AWS Bedrock (KI-Bildgenerierung):</strong> Zur Erstellung von Bildern nutzen wir den
  Dienst AWS Bedrock. Ihre Eingabedaten (Prompts) werden an diesen Dienst &uuml;bertragen, um das
  gew&uuml;nschte Bild zu generieren. Gem&auml;&szlig; den AWS-Datenschutzbestimmungen f&uuml;r
  Bedrock werden diese Daten standardm&auml;&szlig; nicht zum Training der zugrunde liegenden
  KI-Modelle verwendet. Die Verarbeitung erfolgt zur Erf&uuml;llung des mit Ihnen geschlossenen
  Nutzungsverh&auml;ltnisses (Art. 6 Abs. 1 lit. b DSGVO).
</p>

<h3>Nutzerverwaltung mit AWS Cognito</h3>
<p>
  Um die Funktionen unserer App (Bildgenerierung) nutzen zu k&ouml;nnen, ist eine Registrierung
  erforderlich. Hierzu nutzen wir den Dienst <strong>AWS Cognito</strong>. Bei der Registrierung und
  beim Login werden die von Ihnen eingegebenen Daten (E-Mail-Adresse, Passwort-Hash) an AWS
  &uuml;bertragen und dort gespeichert. Dies erfolgt zum Zwecke der Identifizierung und
  Authentifizierung der Nutzer auf Grundlage von Art. 6 Abs. 1 lit. b DSGVO
...
<p>
  ...
  Anfrage und f&uuml;r den Fall von Anschlussfragen bei uns gespeichert. Diese Daten geben wir nicht
